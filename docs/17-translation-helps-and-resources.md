...
**Shipped (v0):** Catalog GL→GL (metadata **`source`** lineage, optional org pins on **`source`** entries), source-first scan, explicit source-set compare, Door43 repo / metadata / bundle links, **`srcOrg`** on **`/translation-helps`**, and **matched-pair manifest file matrix**: `compareGlToGlTcReadyBookProjects` loads catalog **metadata** and **recursive `git/trees`** for each side, then diffs **blob paths** under every **`projects[].path`** root (so e.g. Translation Words / Academy compare actual files under `bible/` or `intro/`, not only identical top-level project folder names). Web: **`/translation-helps?matrix=1`**. **Not shipped yet:** per-ingredient / verse-level matrix (TSV rows, anchors), interlink-derived checklists, and **tag vs `master` diffs** — see `@biblia-studio/translation` + `@biblia-studio/door43` (+ `formats` when parsing is required). Contract detail: [Translation helps domain API § 4.2](./18-translation-helps-domain-api.md).
...
**Book matrix (manifest file slice):** `compareGlToGlTcReadyBookProjects(...)` runs on **matched** GL→GL pairs. For each pair it fetches **`GET .../catalog/metadata/...`** for **source** and **target**, and **`GET .../repos/{owner}/{repo}/git/trees/{ref}?recursive=true`** for **both** repos (`fetchDoor43RepoGitTree` in `@biblia-studio/door43`). It keeps **blob** paths that fall under the directory or file roots listed in **`manifest.projects[].path`**, then diffs those sets (**`pathsInBoth`**, **`pathsOnlyInSource`**, **`pathsOnlyInTarget`** in the domain API / web port). **`apps/web`:** **`/translation-helps`** with GL→GL compare and **`matrix=1`** (optional **`matrixMax`** 1–40, default 15 matched pairs). This is **file inventory** per resource at the catalog tag—not verse-level or per-TSV-row parity (see [end-user outcomes](#end-user-outcomes-target-experience)).
...
| [`@biblia-studio/translation`](./02-package-map.md) | **GL→GL:** `compareGlToGlTcReadyTranslationHelps`; **`findTargetCatalogEntriesClaimingSource`** / **`compareTcReadySourceResourcesToTarget`** (metadata **`source`**, org when pinned). **Book matrix:** `compareGlToGlTcReadyBookProjects` (metadata + **`git/trees`** blob paths under **`projects[].path`**). **Planned:** full **book × help** state matrix, **release tag vs default branch** WIP ([end-user outcomes](#end-user-outcomes-target-experience)). |
