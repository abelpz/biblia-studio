...
**Source-first scan:** `findTargetCatalogEntriesClaimingSource({ targetLanguage, sourceLanguage, sourceIdentifier, ... })` lists **target** tc-ready rows whose catalog **`dublin_core.source`** claims that lineage. **`apps/web`** exposes this on **`/translation-helps`** when both **`srcLang`** and **`srcId`** query params are set (**`lang`** = target catalog language; optional **`srcVer`**); **GL→GL compare** (`compare` ≠ `lang`) takes precedence over source-first vs plain catalog. For rows with **`owner` / `name`** (and ref) from catalog search, the web adapter also fills **Gitea repo** and **`GET .../catalog/metadata/...`** links on the catalog, source-first, and GL→GL views.
...
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`source`**                | Array of **`{ identifier, language, version }`** describing **upstream lineages** (e.g. an **`es-419`** **`glt`** may list **`{ identifier: "ult", language: "en", version: "39" }`** — i.e. translated from English **ULT** at that source **version**). Match target ↔ source GL scripture by **same identifier + language** in **`source`**, then compare **version numbers** to see if the target needs a refresh from a newer baseline. |
...
**In code:** `@biblia-studio/door43` exposes **`fetchDoor43CatalogMetadata`** / **`parseCatalogMetadata`** (`catalogGetMetadata`). **`listTcReadyTranslationHelpsResources`** summaries can include **`catalogOwner`**, **`catalogRepo`**, **`catalogRef`** for metadata URLs. **`@biblia-studio/translation`** adds **`findTargetCatalogEntriesClaimingSource`** (target tc-ready rows whose metadata **`dublin_core.source`** matches a given **`language` + `identifier`**). The symmetric **`compareGlToGlTcReadyTranslationHelps`** prototype still pairs by **`subject` + `identifier`** only; use metadata **`source`** when abbreviations differ or for scripture lineages.
...
The current **`compareGlToGlTcReadyTranslationHelps`** helper is a **symmetric catalog diff**; a **source-driven** tool should iterate **your source list**, resolve targets via **`source`**, then call comparison **only** where step 3 applies.
...
**Goal:** Give **developers** building **translation helps production** and **project management** tools a **stable, documented surface** (REST, RPC, or library — **to be chosen in an issue/ADR**) so **end-user outcomes** ([above](#end-user-outcomes-target-experience)) are achievable:
