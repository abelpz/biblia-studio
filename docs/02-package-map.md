...
| `@biblia-studio/translation` | `packages/translation` | **Translation** workflow concepts: checks, progress, roles, handoffs — without owning full UI. **Catalog:** **`compareGlToGlTcReadyTranslationHelps`** (symmetric diff); **`findTargetCatalogEntriesClaimingSource`** (metadata **`source`**). **Doc target:** **book** + **helps** matrix; **tag vs default branch** WIP — [Translation Helps](./17-translation-helps-and-resources.md). |
