...
| **Translation** | **`compareGlToGlTcReadyTranslationHelps`**, **`compareGlToGlTcReadyBookProjects`** (manifest **`projects`** book ids), **`findTargetCatalogEntriesClaimingSource`** + tests + README                                                                                   | Per-ingredient / verse-level matrix, tag vs default-branch WIP                           |
...
| **Apps/web**    | Door43 repo list (M1), `/editor` (M2), **`/translation-helps`** (catalog, GL→GL, source-first, Door43 links, bundle; **book matrix** `matrix=1` / `matrixMax`) — package `translation`                                                                                 | Richer matrix UI (ingredients), tag vs default-branch parity                             |
